...
public class NeoUtils{
    ...
    // Project Unity (reborn on our.)
    public static final Interp.PowIn pow6In = new Interp.PowIn(6);
    ...
    private static final Seq<NeoUtils.Hit> hitEffects = new Seq<>();
    ...
    public static void init(){
        Events.on(EventType.WorldLoadEvent.class, event -> collideLineCollided.resize(world.width(), world.height()));
    }
    ...
    /** @author EyeOfDarkness */
    public static void collideLineRawEnemy(Team team, float x, float y, float x2, float y2, float width, boolean hitTiles, boolean hitUnits, boolean stopSort, NeoUtils.HitHandler handler){
    ...
    /** @author EyeOfDarkness */
    public static void collideLineRaw(float x, float y, float x2, float y2, float unitWidth, float tileWidth, Boolf<Building> buildingFilter, Boolf<Unit> unitFilter, boolean hitTile, boolean hitUnit, Floatf<Healthc> sort, NeoUtils.HitHandler hitHandler, boolean stopSort){
        hitEffects.clear();
        lineCast.clear();
        lineCastNext.clear();
        collidedBlocks.clear();
                ...
                lineCast.each(i -> {
                    ...
                    if(build != null && (buildingFilter == null || buildingFilter.get(build)) && collidedBlocks.add(build.pos())){
                        if(sort == null){
                            ...
                            NeoUtils.Hit he = Pools.obtain(NeoUtils.Hit.class, NeoUtils.Hit::new);
                            he.ent = build;
                            he.x = tx * tilesize;
                            he.y = ty * tilesize;
